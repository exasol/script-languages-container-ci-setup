...
def run_deploy_ci_build(aws_access: AwsAccess, project: str, github_url: str):
    ...
    """
    logging.info(
        f"run_deploy_ci_build for aws profile {aws_access.aws_profile} for project {project} at {github_url}"
    )
    dockerhub_secret_arn = aws_access.read_dockerhub_secret_arn()
    yml = render_template(
        CI_CODE_BUILD_TEMPLATE,
        project=project,
        dockerhub_secret_arn=dockerhub_secret_arn.aws_arn,
...
def run_validate_ci_build(aws_access: AwsAccess, project: str, github_url: str):
    ...
    """
    logging.info(
        f"run_validate_ci_build for aws profile {aws_access.aws_profile} "
