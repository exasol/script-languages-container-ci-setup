...
def run_deploy_source_credentials(
    ...
    """
    logging.info(f"run_deploy_source_credentials for aws profile {aws_access.aws_profile}")
...
def run_validate_source_credentials(aws_access: AwsAccess,
                                    secret_name: str,
                                    secret_user_key: str,
                                    secret_token_key: str):
    """
    This command deploys the source credential as cloudformation stack.
    """
    logging.info(f"run_validate_source_credentials for aws profile {aws_access.aws_profile}")
    yml = render_template("slc_source_credential.yaml",
                          secret_name=secret_name,
                          secret_user_key=secret_user_key,
                          secret_token_key=secret_token_key)
