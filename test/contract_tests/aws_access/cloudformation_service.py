...
class ValidateContractContract:
    ...
    def validation_result(
        self, cloudformation_service, template_body
    ) -> ValidationResult:
        validation_result = cloudformation_service.validate_template(
...
class ListStackResourcesContractContract:
    ...
    @pytest.fixture(scope="class")
    def list_stack_resources_result(
        self, cloudformation_service, stack_name
    ) -> ListStackResourcesResult:
        list_stack_resources_result = cloudformation_service.list_stack_resources(
            stack_name=stack_name, next_token=None
        )
        return list_stack_resources_result

    def test_stack_resource_summaries_count(
        self, list_stack_resources_result, number_of_resources
    ):
        assert (
            len(list_stack_resources_result.stack_resource_summaries)
            == number_of_resources
        )

    def test_physical_resource_ids_is_not_none(self, list_stack_resources_result):
        resources_with_physical_resource_id = [
            stack_resource_summary
            for stack_resource_summary in list_stack_resources_result.stack_resource_summaries
    ...
    def test_resource_type(self, list_stack_resources_result, resource_type):
        resources_with_given_resource_type = [
            stack_resource_summary.physical_resource_id
            for stack_resource_summary in list_stack_resources_result.stack_resource_summaries
