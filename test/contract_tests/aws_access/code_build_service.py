...
from exasol_script_languages_container_ci_setup.lib.aws.wrapper.datamodels.codebuild import BuildBatchStatus, BuildBatch
...
class StartCodeBuildContract:
    ...
    @pytest.fixture(scope="class")
    def build_batch(self, codebuild_service, project_name, source_version,
                    environment_variables_override) -> BuildBatch:
...
class BatchGetBuildBatchesWithBatchIdFromStartBuildCode:
    ...
    @pytest.fixture(scope="class")
    def build_batches(self, codebuild_service, project_name, source_version,
                      environment_variables_override) -> Tuple[BuildBatch, BuildBatch]:
        build_batch_from_start = codebuild_service.start_build_batch(
            project_name=project_name,
            source_version=source_version,
            environment_variables_override=environment_variables_override)
        build_batches_from_get = codebuild_service.batch_get_build_batches(build_batch_ids=[build_batch_from_start.id])
...
class BatchGetBuildBatchesSingleBuildIdContract:
    ...
    def build_batches(self, codebuild_service, build_batch_id) -> List[BuildBatch]:
        build_batches = codebuild_service.batch_get_build_batches(build_batch_ids=[build_batch_id])
...
class BatchGetBuildBatchesSingleBuildIdInProgressContract(BatchGetBuildBatchesSingleBuildIdContract):

    def expected_build_batch_status(self) -> BuildBatchStatus:
        return BuildBatchStatus.IN_PROGRESS


class BatchGetBuildBatchesSingleBuildIdSucceededContract(BatchGetBuildBatchesSingleBuildIdContract):

    def expected_build_batch_status(self) -> BuildBatchStatus:
        return BuildBatchStatus.SUCCEEDED


class BatchGetBuildBatchesSingleBuildIdFailedContract(BatchGetBuildBatchesSingleBuildIdContract):

    def expected_build_batch_status(self) -> BuildBatchStatus:
        return BuildBatchStatus.FAILED


class BatchGetBuildBatchesSingleBuildIdStoppedContract(BatchGetBuildBatchesSingleBuildIdContract):
