...
class BatchGetBuildBatchesTestSetup:
    ...
    service = CodeBuildService(internal_aws_client=internal_aws_client)
    build_batch_ids = [
        PhysicalResourceId(aws_physical_resource_id="1"),
        PhysicalResourceId(aws_physical_resource_id="2"),
    ]
    build_batches = service.batch_get_build_batches(
        build_batch_ids=build_batch_ids, from_boto=from_boto
    )
...
def test_batch_get_build_batches_internal_aws_client():
    ...
    assert setup.internal_aws_client.mock_calls == [
        call.batch_get_build_batches(ids=setup.ids)
    ]
...
def test_batch_get_build_batches_from_boto():
    ...
    assert setup.from_boto.mock_calls == [
        call(return_value)
        for return_value in setup.batch_get_build_batches_return_values
    ]
...
class StartBuildBatchTestSetup:
    ...
    mock_cast(internal_aws_client.start_build_batch).return_value = (
        start_build_batch_return_values
    )
    from_boto = Mock()
    ...
    build_batch = service.start_build_batch(
...
def test_start_build_batch_internal_aws_client():
    ...
    assert setup.internal_aws_client.mock_calls == [
        call.start_build_batch(
            projectName=setup.projectName.aws_physical_resource_id,
