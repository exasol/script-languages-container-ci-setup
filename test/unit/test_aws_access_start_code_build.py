...
from exasol_script_languages_container_ci_setup.lib.run_start_build import (
    get_environment_variable_override,
)
...
class TestSetup:
    def __init__(self, batch_get_build_batches_return_values: List[List[BuildBatch]]):
        ...
        self.code_build_service: Union[MagicMock, CodeBuildService] = create_autospec(
            CodeBuildService
        )
        mock_cast(self.code_build_service.batch_get_build_batches).side_effect = (
            self.batch_get_build_batches_return_values
        )
        self.started_build_batch = BuildBatch(
            id=PhysicalResourceId("id"), build_batch_status=None
        )
        ...
        mock_cast(self.aws_client_mock.create_codebuild_service).side_effect = [
            self.code_build_service
        ]

        self.aws_client_factory_mock: Union[MagicMock, AwsClientFactory] = (
            create_autospec(AwsClientFactory)
        )
        mock_cast(self.aws_client_factory_mock.create).side_effect = [
            self.aws_client_mock
        ]

        self.profile = "profile"
        self.aws_access = AwsAccess(
            aws_profile=self.profile, aws_client_factory=self.aws_client_factory_mock
        )
        self.environment_variables_overrides = (
            self._create_environment_variables_overrides()
        )
...
def test_success_aws_client_factory_mock_call():
    setup = TestSetup(create_batch_get_build_batches_return_values_for_success())
    setup.run_start_codebuild()
    assert setup.aws_client_factory_mock.mock_calls == [
        call.create(profile=setup.profile)
    ]
...
def test_success_sleep_function_mock_call():
    setup = TestSetup(create_batch_get_build_batches_return_values_for_success())
    setup.run_start_codebuild()
    assert setup.sleep_function_mock.mock_calls == [call(1), call(1)]
