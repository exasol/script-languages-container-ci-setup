...
class ValidateTemplateSetup:
    ...
    internal_aws_client_validate_template_return_value = mock_cast(internal_aws_client.validate_template).return_value


def test_validate_template_internal_aws_client():
    setup = ValidateTemplateSetup()
    assert setup.internal_aws_client.mock_calls == [call.validate_template(TemplateBody=setup.template_body)] \
           and setup.from_boto.mock_calls == [call(setup.internal_aws_client_validate_template_return_value)] \
           and setup.validation_result == setup.from_boto.return_value


def test_validate_template_from_boto():
    setup = ValidateTemplateSetup()
    assert setup.from_boto.mock_calls == [call(setup.internal_aws_client_validate_template_return_value)]
...
class ListStackResourcesTestSetup:
    def __init__(self, next_token: Optional[NextToken]):
        ...
        self.from_boto = Mock()
        self.service = CloudFormationService(internal_aws_client=self.internal_aws_client)
        ...
        self.internal_aws_client_list_stack_resources_return_value = mock_cast(
            self.internal_aws_client.list_stack_resources).return_value
...
def test_list_stack_resources_from_boto(next_token):
    setup = ListStackResourcesTestSetup(next_token)
    assert setup.from_boto.mock_calls == [call(setup.internal_aws_client_list_stack_resources_return_value)]
